...
def _filter_for(cls, raw: dict[str, Any]) -> dict[str, Any]:
...
def _default_text_strings(dataset_name: str, label_values: list[int]) -> list[str]:
    ...
    dataset_map = {"mnist": mnist_map, "cifar10": cifar10_map}.get(dataset_name, {})
...
def _normalize_text_config(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return _filter_for(TextConfig, text_raw)


def _normalize_train_config(raw_train: dict[str, Any]) -> dict[str, Any]:
    normalized = _filter_for(TrainConfig, dict(raw_train))
...
def _normalize_sampling_config(raw_sampling: dict[str, Any]) -> dict[str, Any]:
    normalized = _filter_for(SamplingConfig, dict(raw_sampling))
    ...
    schedule = normalized["image_to_text_text_time_schedule"]
    if schedule not in {"power", "logit_normal"}:
    ...
    scale = float(normalized["image_to_text_logit_normal_scale"])
    if scale <= 0.0:
        raise ValueError(f"image_to_text_logit_normal_scale must be > 0, got {scale}")
    normalized["image_to_text_logit_normal_scale"] = scale
    return normalized
...
def load_config(path: str | Path) -> ProjectConfig:
    ...
    return ProjectConfig(
        ...
        repo_root=repo_root,
        paths=paths,
        tokenizer=TokenizerConfig(**_filter_for(TokenizerConfig, raw["tokenizer"])),
        ...
        eval=EvalConfig(**_filter_for(EvalConfig, raw["eval"])),
    )
