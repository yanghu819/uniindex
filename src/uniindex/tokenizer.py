...
def _resolve_local_model_source(model_name: str, cache_dir: Path, repo_root: Path) -> tuple[str, str | None]:
    ...
    raise FileNotFoundError(
        f"missing local vision tokenizer model '{model_name}'. Expected a complete local snapshot under one of: "
        + ", ".join(searched)
        + ". This loader is offline-only; populate the repo-local cache before running."
    )
